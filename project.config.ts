 ...
 * 实现了默认值和路径计算的封装
 */
class ProcessConfig implements IProcessConfig {
  public root: string;
  public entry: string;
 ...
 * 💡 Electron 路径配置接口
 */
interface IElectronConfig<P extends boolean = false> {
  /**
   * Electron 源码根目录
   * @default 'electron'
   */
 ...
 * 💡 Electron 路径配置类
 */
class ElectronConfig implements IElectronConfig {
  public root: string;
  public dist: string;
 ...
 * 💡 Renderer 路径配置类
 */
class RendererConfig implements IRendererConfig {
  public root: string;
  public dist: string;
  public entry: string;
